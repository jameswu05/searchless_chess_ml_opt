...
def train(
  ...
  params = predictor.initial_params(
      jrandom.PRNGKey(predictor_config.seed),
...
def train(
  ...
  # Sharding — gracefully handle single device and older JAX versions.
  use_sharding = False
  sharding = None
  if jax.device_count() > 1:
    try:
      ...
      logging.warning('Sharding unavailable, running on single device: %s', e)
